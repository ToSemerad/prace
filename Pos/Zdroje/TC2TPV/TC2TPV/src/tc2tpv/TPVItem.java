...
public class TPVItem {
    ...
    public final static int TYPE_SPECIAL_TOOL = 5; // Speciální nářadí
    ...
    private final HashMap<String, Object> externalParameters;
    ...
    public void setParameter(String TCName, String value) throws SQLException, InvalidParameterException, InvalidParameterTypeException {
        ...
        if (rs.next()) {
            ...
            if (nameTPV.startsWith("ext_")) {
                this.externalParameters.put(nameTPV, value);
            } else {
    ...
    public void setParameter(String TCName, Object value) throws SQLException, InvalidParameterException, InvalidParameterTypeException {
        ...
        if (rs.next()) {
            ...
            if (typTPV.equals("S")) {
                if (nameTPV.startsWith("ext_")) {
                    this.externalParameters.put(nameTPV, String.valueOf(value));
                } else {
            ...
            if (typTPV.equals("I")) {
                try {
                    if (nameTPV.startsWith("ext_")) {
                        this.externalParameters.put(nameTPV, Integer.valueOf(value.toString().replace(",", ".")));
                    } else {
            ...
            if (typTPV.equals("F")) {
                try {
                    if (nameTPV.startsWith("ext_")) {
                        this.externalParameters.put(nameTPV, Float.valueOf(value.toString().replace(",", ".")));
                    } else {
     ...
     */
    public void addBomLine(TPVItem item, float quantity, int position) throws PositionAlreadyExistsException, InvalidPositionException {
        TPVBOMLine bl = new TPVBOMLine(item, quantity, position);
        
        for (TPVBOMLine l : bomLines) {
            if (l.getPosition() == position) {
                throw new PositionAlreadyExistsException(position, l.getItem().itemId);
            }
        }
        
        if (position <= 0 || position >= 5000) {
            throw new InvalidPositionException(itemId);
        }
    ...
    public int insertItemToTPV() throws SQLException, ConfigurationNotFoundException {
        ...
        if (rs.next()) {
            ...
            TPVItem.linno = 0;
            insertItem(klicImportu, 0, "", 1, 1, 0);
            return klicImportu;
        }
        return 0;
    }
     ...
     * @throws SQLException
     */
    public void insertItem(int importId, int linno_v, String id_v, int level, float mnozstvi, int position) throws SQLException {
        TPVItem.linno++;
        String qIns = "INSERT INTO tpv_c2t_import (klic_imp_hlav, stav, linno, linno_v, uroven, alter_nazev_v, mnozstvi, alter_nazev, nazev_polozky, postaveni, index_vykresu, compseqno, ";
        if (changenum != null && !changenum.equals("")) {
            qIns = "INSERT INTO tpv_c2t_import (klic_imp_hlav, stav, linno, linno_v, uroven, alter_nazev_v, mnozstvi, alter_nazev, nazev_polozky, postaveni, index_vykresu, changenum, compseqno, ";
        }
        ...
        int i;
        if (changenum != null && !changenum.equals("")) {
            ps.setString(12, changenum);
            ps.setInt(13, position);
            i = 14;
        } else {
            ps.setInt(12, position);
            i = 13;
        }
        for (String key : parameters.keySet()) {
        ...
        if (externalParameters.size() > 0) {
            String sqlext = "INSERT INTO tpvg_c2t_import_ext (klic_import, ";
            for (String key : externalParameters.keySet()) {
                sqlext += key.replace("ext_", "") + ", ";
            }
            ...
            sqlext = sqlext.substring(0, sqlext.length() - 2);
            PreparedStatement psext = TPVSQL.getConnection().prepareStatement(sqlext);
            psext.setInt(1, klicImportu);
            i = 2;
            for (String key : externalParameters.keySet()) {
                Object value = externalParameters.get(key);
                
                if (value instanceof String) {
                    psext.setString(i, (String) value);
                } else if (value instanceof Float) {
                    psext.setFloat(i, (Float) value);
                } else if (value instanceof Integer) {
                    psext.setInt(1, (Integer) value);
                }
        ...
        if (!this.files.isEmpty()) {
            ...
            int radek = 1;
            for (File f : files) {
                PreparedStatement psf = conn.prepareStatement("INSERT INTO tpvg_c2t_import_dok (klic_importu, rad_importu, soubor, extenze, stav) SELECT ?, ?, ?, ?, ?");
        ...
        for (TPVBOMLine line : bomLines) {
            TPVItem lowerItem = line.getItem();
            lowerItem.insertItem(importId, lnv, this.itemId, level + 1, line.getQuantity(), line.getPosition());
        }
    }
