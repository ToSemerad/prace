...
static void do_it(char* id, char* item_name, char* WF_template)
{
    int 
        attach_types[1] = {1};
    tag_t 
    ...
    ITEM_find_item(id, &tag_item);
    EXIT_IF_NULL( tag_item);

    ERROR_CHECK( EPM_find_process_template(WF_template, &process_template) ); 
    EXIT_IF_NULL( process_template );


    ERROR_CHECK( EPM_create_process(item_name,  "spusteno importem zakazky", process_template, 1, &tag_item, 
...
static void start_process(tag_t obj)
{
	printf(">>>>>>>>>start process \n");
	//int
	//	attach_types[1] = {EPM_target_attachment},//EPM_interprocess_task_attachment //EPM_target_attachment //EPM_user_attachment  
	//	count;
	//tag_t
	//	process_template = NULLTAG,
	//	//attachment[1]={obj},
	...
	//ERROR_CHECK(EPM_find_process_template("Xtest_TS", &process_template));//create_process//00_PROVEDENI_ZMENY
//	EXIT_IF_NULL(process_template);
	//ERROR_CHECK(ITEM_ask_name2	(obj,&item_name));
	//ERROR_CHECK(ITEM_ask_id2(obj,&id_obj));

	//AOM_refresh(obj,true);

	/*char cmd[128]="C:\\SPLM\\Apps\\Import_zmeny\\StratWFProcess.exe -i=";
	strcat(cmd,id_obj);
	printf("%s \n",cmd);
	system(cmd);*/

	//printf("item_name= %s, id %s , templete 00_PROVEDENI_ZMENY \n",item_name,id_obj);
	 


	//ERROR_CHECK(EPM_create_process(item_name, "spusteno importem zakazky", process_template, 1,attachment,attach_types, &process)); //create_process
	//ERROR_CHECK(EPM_create_process_deferred_start(item_name, "spusteno importem zakazky", process_template, 1,obj_add,attach_types, &process)); //create_process
	//ERROR_CHECK(EPM_create_process(item_name, "spusteno importem zakazky", process_template, 1,attachment,attach_types, &process));

	//do_it(id_obj,item_name, "00_PROVEDENI_ZMENY");

	/*int err=EPM_create_process(item_name, "spusteno importem zakazk", process_template, 1,obj_add,attach_types, &process);
	printf ("err = %d \n",err);
	EMH_clear_errors();
	EXIT_IF_NULL(process);*/
}
...
int Existence(char hodnoty [20][256],char* typ, int typ_pol_num)
	{
			...
			const char *Names[1] = { "object_type"};
				...
				IFERR_REPORT(QRY_find(QueryExistenece[typ_pol_num], &query));
				...
				printf("hodnota hledani %s delka %s ATTR %s\n",entries[0],values[0],entries[0]);						
				IFERR_REPORT(QRY_execute(query, 1, entries, values, &n_item, &item));
