...
extern "C" DLLAPI int TPV_CreateName_HM_NP_TC11_register_callbacks();
extern "C" DLLAPI int TPV_CreateName_HM_NP_TC11_init_module(int *decision, va_list args);
...
extern "C" DLLAPI int TPV_CreateName_HM_NP_TC11_register_callbacks()
{
    printf("Registruji TPV_CreateName_HM_NP_TC11.dll\n");
    CUSTOM_register_exit("TPV_CreateName_HM_NP_TC11", "USER_init_module", TPV_CreateName_HM_NP_TC11_init_module);
...
char* Get_Lov(char* hodnota,char* Lov )
{
	...
	for(int j=0; j<n_values;j++)
	{
		...
		printf("cislo %d hodnoty %s vstup %s popis %s \n",j,values[j],hodnota,values_dissplay[j]);
...
char* tmp,										
	...
	if(strlen (tmp)>1)
	{
	printf("Polotovar tmp %s \n",tmp);
	//printf("Get polotovar %s \n",Get_Lov(tmp,"TPV4_TYP_POLOTOVARU"));

	//printf("polotovar get%s \n",tmp);
	strcat(new_name,Get_Lov(tmp,"TPV4_TYP_POLOTOVARU"));
	}
	strcat(new_name,"|");
	AOM_get_value_double(obj,"tpv4_rozmer1",&tmp_d);
	if(tmp_d!=0)
	{
	...
	strcat(new_name,"|");
	printf("delka %d \n",strlen(new_name));
	AOM_get_value_double(obj,"tpv4_rozmer2",&tmp_d);
	if(tmp_d!=0)
	{
		sprintf(tmp,"%.2f",tmp_d);
		strcat(new_name,tmp);
	...
	strcat(new_name,"|");
	printf(" 200 delka %d \n",strlen(new_name));
	AOM_get_value_double(obj,"tpv4_rozmer3",&tmp_d);
	if(tmp_d!=0)
	{
		sprintf(tmp,"%.2f",tmp_d);
		strcat(new_name,tmp);
...
char* tmp,										
	...
	AOM_get_value_string(obj,"tpv4_rozmer",&tmp);
	strcat(new_name,tmp);
	strcat(new_name,"|");
...
//printf("test0 \n");		
    ...
    EPM_ask_attachments( RootTask,EPM_target_attachment, &TargetsCount, &Targets );// z knihovny epm.h "#define EPM_target_attachment               1        /**< Target attachment type */"
							...
							printf("tag Query STS_HUT_MATERIAL je %d\n",query);
							...
							printf("tag Query STS_NAK_POLOZKA je %d\n",query);
