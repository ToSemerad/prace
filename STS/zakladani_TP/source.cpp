...
extern "C" DLLAPI int TPV_Create_I_TP_TC11_register_callbacks();
extern "C" DLLAPI int TPV_Create_I_TP_TC11_init_module(int *decision, va_list args);
...
extern "C" DLLAPI int TPV_Create_I_TP_TC11_register_callbacks()
{
    printf("Registruji handler-TPV_Create_I_TP_TC11.dll\n");
    CUSTOM_register_exit("TPV_Create_I_TP_TC11", "USER_init_module", TPV_Create_I_TP_TC11_init_module);
...
int Existence(char* povrch1, char* povrch2, char* povrch3, char* stredisko, char* poznamka,char* id, char* rev, char* var,tag_t BomLine,int AttributeId,tag_t BomWindow, tag_t revDil)
	{
			...
			char* TP=NULL;
			char* id_x=NULL;
			char Idd[ITEM_id_size_c + 1];
			char Namee[ITEM_name_size_c + 1];
			tag_t revItemu;
				...
				if(ItemsCountt == 0)
				{
					...
					{printf("\n Nalezeno %d položek\n", ItemsCountt);
						...
						for(int i = 0; i < ItemsCountt; i++)
							{	
								ITEM_ask_id(Itemss[i], Idd);
								ITEM_ask_name(Itemss[i], Namee);
								...
								//if( strcmp(povrch1,povrch1_x)==0 && strcmp(povrch2,povrch2_x)==0 && strcmp(povrch3,povrch3_x)==0 && strcmp(poznamka,poznamka_x)==0 && strcmp(id,id_x)==0 ) 
									if( strcmp(povrch1,povrch1_x)==0 && strcmp(povrch2,povrch2_x)==0 && strcmp(povrch3,povrch3_x)==0 && kontrolaLov(stredisko, "H4_stredisko", stredisko_x)==1 && strcmp(poznamka,poznamka_x)==0 && strcmp(id,id_x)==0 ) 
									{
										...
										{
											TP="NULL";
											printf("neshoduji se p1 a p1_xdelka1 je %d delka je %d tedy TP %s \n",strlen(id),strlen(id_x),TP);
										}
								//if(strlen(TP)==8)
							}
				}
