...
extern "C" DLLAPI int TPV_Rename_Export_TC14_register_callbacks();
extern "C" DLLAPI int TPV_Rename_Export_TC14_init_module(int *decision, va_list args);
...
extern "C" DLLAPI int TPV_Rename_Export_TC14_register_callbacks()
{
   ECHO(("Registruji TPV_Rename_Export_TC14.dll\n"));
    CUSTOM_register_exit("TPV_Rename_Export_TC14", "USER_init_module", TPV_Rename_Export_TC14_init_module);
	...
	while(ArgumentCount--> 0 )
{
	...
	//strcat(job_name,Output);
		...
		//strcat(text, " \"\\\\SRVTCBASE\\TcESO_vymena\\TC_Export\\");
		strcat(text, " \"D:\\TcESO_vymena\\TC_Export\\");
	

		if(copy_tp)
		{
			//strncat(text,job_name,15);
			strcat(text, remove_chars(token));
			strcat(text, "~TP");
			strcat(text, ".plmxml\"");
		}
		if(copy_kp)
