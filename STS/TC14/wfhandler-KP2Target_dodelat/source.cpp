...
extern "C" DLLAPI int TPV_Refs2Targets_TC14_register_callbacks();
extern "C" DLLAPI int TPV_Refs2Targets_TC14_init_module(int *decision, va_list args);
int TPV_Refs2Targets_TC14(EPM_action_message_t msg);
//EPM_decision_t RhTest(EPM_rule_message_t msg);
void ListBomLine(tag_t BomLine, int Level, tag_t RootTask);
extern "C" DLLAPI int TPV_Refs2Targets_TC14_register_callbacks()
{
   ECHO(("Registruji Handler-TPV_Refs2Targets_TC14.dll\n"));
    CUSTOM_register_exit("TPV_Refs2Targets_TC14", "USER_init_module", TPV_Refs2Targets_TC14_init_module);
...
extern "C" DLLAPI int TPV_Refs2Targets_TC14_init_module(int *decision, va_list args)
{
    *decision = ALL_CUSTOMIZATIONS;  // Execute all customizations
    ...
    int Status = EPM_register_action_handler("TPV_Refs2Targets_TC14", "", TPV_Refs2Targets_TC14);
    if(Status == ITK_ok)ECHO(("Handler pro pridani TP do targetu %s byl registrován\n", "TPV_Refs2Targets_TC14"));
...
void ListBomLine(tag_t BomLine, int Level, tag_t RootTask)
{
   ...
   // BOM_line_ask_attribute_string(BomLine, AttributeId, &TP);
	AOM_ask_value_string(BomLine, "bl_TPV4_dilRevision_tpv4_cislo_tp", &TP);//this is bad thing
