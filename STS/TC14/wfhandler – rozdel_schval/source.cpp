...
extern "C" DLLAPI int TPV_Rozdel_schaval_zak_TC14_register_callbacks();
extern "C" DLLAPI int TPV_Rozdel_schaval_zak_TC14_init_module(int *decision, va_list args);
...
extern "C" DLLAPI int TPV_Rozdel_schaval_zak_TC14_register_callbacks()
{
    ECHO(("Registruji handler-TPV_Rozdel_schaval_zak_TC14.dll\n"));
    CUSTOM_register_exit("TPV_Rozdel_schaval_zak_TC14", "USER_init_module", TPV_Rozdel_schaval_zak_TC14_init_module);
...
void ListBomLine(tag_t BomLine, int Level, tag_t RootTask, tag_t BomWindow,char* user_name)
{
 ...
 //   BOM_line_look_up_attribute("bl_revision", &AttributeId);
	...
	}else 
	{
		...
		ECHO(("procet schavlenych kusovniku %d \n",Count));
...
void AddToTarget(tag_t RootTask,char* O_Name,tag_t Item)
{
	ECHO(("tak posilam do Targetu Roottask %d Item %f jmeno %s \n",RootTask,Item,O_Name));
