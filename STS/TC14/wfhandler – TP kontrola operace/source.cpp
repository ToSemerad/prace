...
extern "C" DLLAPI int TPV_Control_OP_TC14_register_callbacks();
extern "C" DLLAPI int TPV_Control_OP_TC14_init_module(int *decision, va_list args);
...
extern "C" DLLAPI int TPV_Control_OP_TC14_register_callbacks()
{
    ECHO(("Registruji handler-TPV_Control_OP_TC14.dll\n"));
    CUSTOM_register_exit("TPV_Control_OP_TC14", "USER_init_module", TPV_Control_OP_TC14_init_module);
	 ...
	 int BomsCount = 0;
        ...
        for(int j = 0; j < BomsCount; j++)
        {
			...
			for (int j=0;j<ChildsCount;j++)
			{
				...
				AOM_ask_value_tag(Childs[j], "bl_revision", &Rev);
