...
extern "C" DLLAPI int TPV_Control_CSV_TC12_register_callbacks();
extern "C" DLLAPI int TPV_Control_CSV_TC12_init_module(int *decision, va_list args);
...
extern "C" DLLAPI int TPV_Control_CSV_TC12_register_callbacks()
{
    printf("Registruji TPV_Control_CSV_TC12.dll\n");
    CUSTOM_register_exit("TPV_Control_CSV_TC12", "USER_init_module", TPV_Control_CSV_TC12_init_module);
...
//printf("test1 \n");
 ...
 strcpy(file,"\0");
// printf(" 168 test po zacatku zapisu %d \n",fp);
//fprintf(fp,attr);
 printf ("line >>>> %s \n",line);
 fprintf(fp,"%s \n",line);
...
char* tmp,										
	...
	strcat(new_name,tmp);
	strcat(new_name,deliver);
	AOM_get_value_double(obj,"tpv4_hmotnost",&tmp_d);
							...
							for (int i=0;i<n_count_hm;i++)
							{
								...
								else if(strcmp(status,"Opravena v TC")==0)
								{
									...
									//	 printf ("tag rev %d \n",rev);
									 ...
									 if(ifail != ITK_ok) {printf("problem schvaleni rev \n");}
