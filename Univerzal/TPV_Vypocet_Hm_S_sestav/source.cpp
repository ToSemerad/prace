				...
				for( int y =0;y<poradi;y++)
					{
						if(Strom[i][2]==Strom[y][2]&&Strom[y][0]==x){
										...
										//	BOM_line_ask_attribute_string(Strom[y][1], AttributeId, &SEAR);
											...
											AOM_get_value_double(Rev,"tpv4_hmotnost",&Hmotnost);
											AOM_get_value_double(Rev,"tpv4_plocha",&Plocha);
											...
											//AOM_get_value_double(RevParent,"tpv4_hmotnost",&mnozstvi);
								...
								if(Strom[i][3]!=1 )
								{
									if((HmS >0))
									{
										...
										printf("max level %d \n",x);
										SetDouble(RevParent,HmS,"tpv4_hmotnost");
									}
									if((PlochaSestavy >0))
									{
										printf("zapis Plochy.....%f \n",PlochaSestavy);
										printf("max level %d \n",x);
										SetDouble(RevParent,PlochaSestavy,"tpv4_plocha");
									}
