...
int  Add_material(char* id_polozky,int *poradi,char* id_rodice,char* obj_name )
{
	...
	int ReadAttr(tag_t Rev, int poradi,char* ID_v, char* mnozstvi)
	{
	...
	//printf ("test305 %s \n",ID_v);
		...
		strcat(id,tmp);
		strcpy(Nazvy[poradi][15],id);//ID_TC
	...
	//	printf("--------Type %s %d-------\n",TypItemu,strcmp(TypItemu,"TPV4_nak_dilRevision"));
		...
		else if(strcmp(TypItemu,"TPV4_dilRevision")==0)
		{
			...
			if(strlen(tmp)>19)
				strncpy(Nazvy[poradi][10],tmp,19);
			else
			...
			if(strcmp(hmotnost,"-")==0)
				strcpy(Nazvy[poradi][12]," ");
			else
				strcpy(Nazvy[poradi][12],hmotnost);
			//poznamka
			char *poznamka;
			...
			if(strcmp(stav,"OVEROVACI SERIE")==0||strcmp(stav,"OS")==0)
				strcpy(Nazvy[poradi][14],"OS");
			else if(strcmp(stav,"ZAKAZKA")==0||strcmp(stav,"ZAK")==0)
				strcpy(Nazvy[poradi][14],"ZAK");//tpv4_stav
			else
				strcpy(Nazvy[poradi][14],"VYR");//tpv4_stav
...
int ListBomLine(tag_t BomLine, int Level, tag_t pamet[], int poradi,tag_t BomWindow,int Strom [6][4])
{
	...
	for(int t=0;t<ChildsCount;t++)
	{
		...
		printf("%d>>>>Quant %s \n",poradi,Quant);
		poradi=ReadAttr(Rev,poradi,I_ID_v,Quant);
		//strcpy(Nazvy[poradi][7],tmp);
			char tmpName[128];
...
for(i=0;i<m;i++)
{
	printf ("fprint %d \n",i);
	if(i==0)
		fprintf(fp,"poradi#idv#id#nazev#revize#mnozstvi#prilohy#material#typ#klic#zmena#tpv4_polotovar#tpv4_material#tpv4_hmotnost#tpv4_poznamka_tpv#tpv4_stav#ID_TC#tpv4_skupina#klic_maxu\n%d",i);
...
for( int i = 0; i < TargetsCount; i ++ )
{
	...
	for(int l=0;l<6;l++){
		...
		create_marks_csv(file_name,poradi,16);
