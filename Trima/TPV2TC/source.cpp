...
int GetAttr(char* attr,int polozka_num)
{
	...
	int num=0;
	for (int i=1;i<Attr_num[polozka_num]+1;i++)
	{
...
void ReadProperty()
{
    ...
    while (fgets(line, 1024, stream))
    {
		...
		{	strcpy(Attr_type[Type_num][Attr_num[Type_num]],tmp);
			tmp=strtok(NULL,"<");
			printf("ItemType attr %s \n",tmp);
			strcpy(Attr[Type_num][Attr_num[Type_num]],tmp);
			Remove_line++;
		}
		...
		else if(strcmp(tmp,"<importPDF")==0)
		{
			strcpy(Attr_type[Type_num][Attr_num[Type_num]],tmp);
			tmp=strtok(NULL,"<");
			//printf("double attr %s \n",tmp);
				...
				else if(strcmp(tmp,"<login")==0)
		{
			...
			tmp=strtok(NULL,"<");
			printf("login %s\n",tmp);
			Attr_Typ_polozky=Attr_num[Type_num];
			strcpy(login,tmp);
			goto nextLine;
		...
		/*if( strcmp(tmp,"<TPV2000")==0)
		{
			tmp=strtok(NULL,"<");
			printf("TPV attr %s \n",tmp);
			strcpy(Attr_tpv[Type_num][Attr_num[Type_num]],tmp);
...
void SetProperty (int polozka_num,int num,tag_t object_type,char* value)
{
	...
	else if(strcmp(typ_polozky,"<importPDF")==0)
	{ 
		printf("Import PDF \n");
		char importPDF[256];
		strcpy(importPDF,"C:\\SPLM\\Apps\\Import_TC_trans\\findPDF.bat \"-findPDF=");
		strcat(importPDF,value);
		strcat(importPDF,";\"");
		printf("find PDF %s \n",importPDF);
		system(importPDF);
	...
	else if(strcmp(typ_polozky,"<relace")==0)
	{
		// doplnit vytvaøení relace
		char* name;
		...
		tag_t late_rev;
		tag_t rev_obj;
		ITEM_find_item(value,&find_item);
		ITEM_ask_latest_rev(find_item,&late_rev);
		 ITEM_ask_rev_name2(late_rev,&name);
		ITEM_ask_latest_rev(object_type,&rev_obj);
		printf("relace %s value %s rev %d rev2 %d \n name_primary %s ",Attr[polozka_num][num],value,late_rev,rev_obj,name);
		create_relation(Attr[polozka_num][num],late_rev,rev_obj);
	}
...
void Create(char hodnoty[50][256],int TypPolozky_num){
	...
	//revise
		...
		if(Existence(hodnoty,typRevize, TypPolozky_num)==1)
		{
			...
			AOM_lock(Item);
			for (int i=0;i<Attr_num[TypPolozky_num]-1;i++)
			{
				if(i!=num_name)
...
//	char *tmp_field;
	...
	for (int i =2;i<=Attr_num[typ]-Remove_line;i++)
	{
...
int Existence(char hodnoty [20][256],char* typ, int typ_pol_num)
	{
			...
			const char *Names[1] = { "object_type"};
				...
				else
				{
					AOM_lock(*item);
					printf("Attr_num %d \n", Attr_num-1);
					for (int i=0;i<Attr_num[typ_pol_num]-1;i++)
					{
						if(i!=num)
