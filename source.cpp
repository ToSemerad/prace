  ...
  while (fgets(line, 7000, stream)) {
	  ...
	  if(stejnyRadek)	//Pokud není na stejném øádku <QZBOZI_TPV> a </QZBOZI_TPV>, spoj tyto øádky
	  {
		...
		printf("\n%d velikost %d z maxima %d \n%s\n", vysledek++, strlen(line), sizeof(line), line);	//strlen(line): èíselný výpis poètu znakù na aktuálním øádku
			if(strlen(line)<=10){																							//sizeof(line): èíselný výpis maximálního poètu znakù na øádku
				printf("Line je moc kratka: %d radek: %d",strlen(line),__LINE__);
				system("pause");
				goto next_line;
			}

    for (int i = 0; i < strlen(line); i++)		//Prochází znaky na øádku po jednom, jejich maximální poèet je 7000. 
	{
		
		if(i > (strlen(line)-14)){	//Pokud je i na konci øetìzce(øádku), nastavit na jeho maximální hodnotu, aby se mohl provést další øádek
		printf("radek: %d pozice: %d znak: %c\n",__LINE__,i,line[i]);
		//i=7000;
		system("pause");
		}
