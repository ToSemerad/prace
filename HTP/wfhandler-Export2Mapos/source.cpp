...
extern "C" DLLAPI int TPV_TC2ERP_TC14_register_callbacks();
extern "C" DLLAPI int TPV_TC2ERP_TC14_init_module(int *decision, va_list args);
int TPV_TC2ERP_TC14(EPM_action_message_t msg);
...
extern "C" DLLAPI int TPV_TC2ERP_TC14_register_callbacks()
{
    ECHO(("Registruji TPV_TC2ERP_TC14.dll\n"));
    CUSTOM_register_exit("TPV_TC2ERP_TC14", "USER_init_module", TPV_TC2ERP_TC14_init_module);
...
int get_string(tag_t object, const char* prop_name, char** value)
{
	char * orig_string;
		...
		LogErrMES("AOM_ask_value_string(object, prop_name, &orig_string)", "TC2ERP_err.log", __LINE__, return_int);
		ECHO(("L:%d %s \n",__LINE__, orig_string));
		*value = orig_string;
...
int copy_string2varacha(char * orig_string,int poradi,int numAttr)
{
	...
	strcat(Nazvy[poradi][numAttr], "'");
	//*value=remove_enter(orig_string);
	ECHO(("%s %d %d \n",Nazvy[poradi][numAttr], poradi,numAttr));
	return return_int;
}
...
extern "C" DLLAPI int TPV_TC2ERP_TC14_init_module(int *decision, va_list args)
{
   ...
   //Registrace action handleru
    int Status = EPM_register_action_handler("TPV_TC2ERP_TC14", "", TPV_TC2ERP_TC14);
...
int ReadAttr_Projekt(tag_t Rev, int poradi, char* ID_v, char* REV_v, char* mnozstvi, char *pozice)
{
	//   0  #     1        #   2  #  3    #  4#  5  #  6   #  7 #      9      #   10
	//ItemID#ItemRevisionID#Nazev#Poznamka#TYP#Autor#Status#Stav#DatZpracovani#Schvaleno
	ECHO(("------------------read Attr ------------------\n"));
	int AttributeId = 0;
	char* tmp;
	char id[32];
	...
	copy_string2varacha(id, poradi, 0);

	get_string(Rev, "item_revision_id", &tmp);
	//strcpy(Nazvy[poradi][2], tmp);//ItemRevisionID
	copy_string2varacha(tmp, poradi, 1);

	get_string(Rev, "object_name", &tmp);
	copy_string2varacha(tmp, poradi, 2);

	get_string(Rev, "object_desc", &tmp);
	//strcpy(Nazvy[poradi][9], tmp);
	copy_string2varacha(tmp, poradi, 3);

	get_string(Rev, "object_type", &TypItemu);
	ECHO(("Type %s \n", TypItemu));
	if (debug) ECHO(("line: %d \n", __LINE__));
	
	if (strcmp(TypItemu, "TPV4_ProjektRevision") == 0 || (strcmp(TypItemu, "TPV4_ZakazkaRevision") == 0))
	{

		if (strcmp(TypItemu, "TPV4_ProjektRevision") == 0)
		{
			if (debug) ECHO(("line: %d \n", __LINE__));
			copy_string2varacha("Projekt", poradi, 4);//strcpy(Nazvy[poradi][3], "V");
			if (debug) ECHO(("line: %d \n", __LINE__));
			
		}
		else if (strcmp(TypItemu, "TPV4_ZakazkaRevision") == 0)
		{
			ECHO(("TPV4_ZakazkaRevision\n"));
			if (debug) ECHO(("line: %d \n", __LINE__));
			copy_string2varacha("Zakazka", poradi, 4);	//	strcpy(Nazvy[poradi][3], "D");
			
		}

		copy_string2varacha(Owner(Rev), poradi, 5);

		copy_string2varacha(status_exportu, poradi, 6);

	
		copy_string2varacha("0", poradi, 7);
		...
		if (is_released == 1)
		{
			...
			copy_string2varacha("TRUE", poradi, 9);

		}
		else
			copy_string2varacha("FALSE", poradi, 9);//strcpy(Nazvy[poradi][18], "FALSE");
		get_string(Rev, "tpv4_zakaznik", &tmp);
		ECHO(("zakaznik %s \n", tmp));
		copy_string2varacha(tmp, poradi, 10);
	...
	if (strcmp(TypItemu, "TPV4_nak_dilRevision") == 0 || strcmp(TypItemu, "TPV4_dilecRevision") == 0|| strcmp(TypItemu, "TPV4_VyrobekRevision") == 0)
		{
			...
			//07.03.2023
			/*get_string(Rev, "tpv4_SZ", &tmp);
			strcpy(Nazvy[poradi][5], tmp);*/

			//07.03.2023
			/*get_string(Rev, "tpv4_SkupinaZbozi", &tmp);
			strcpy(Nazvy[poradi][6], tmp);*/

			//string2number(poradi, 11, tmp);
			/*char* value_x=strtok(tmp," ");
			char* unit_x=strtok(NULL, "\0");
			if (strcmp(tmp, "-") == 0)
				strcpy(Nazvy[poradi][11], "0");
			else
			   strcpy(Nazvy[poradi][11],replace_char(value_x,',', '.'));*/

			   //07.03.2023
		/*	get_string(Rev, "tpv4_RegCisCele", &tmp);
			strcpy(Nazvy[poradi][7], tmp);


			get_string(Rev, "tpv4_RegCis", &tmp);
			strcpy(Nazvy[poradi][8], tmp);

			get_string(Rev, "tpv4_nazev2", &tmp);
			strcpy(Nazvy[poradi][10], tmp);

			get_string(Rev, "tpv4_MaterialJakost", &tmp);
		...
		/*	double hustota;
			...
			//07.03.2023
			/*get_string(Rev, "tpv4_polotovar", &tmp);
			strcpy(Nazvy[poradi][17], tmp);

			get_string(Rev, "tpv4_MJ", &tmp);
			strcpy(Nazvy[poradi][18], tmp);

			get_string(Rev, "tpv4_StareCV", &tmp);
			strcpy(Nazvy[poradi][19], tmp);

			get_string(Rev, "tpv4_SkladovyProces", &tmp);
			strcpy(Nazvy[poradi][20], tmp);

			get_string(Rev, "tpv4_KodPrvotniOperace", &tmp);
			strcpy(Nazvy[poradi][21], tmp);

			double sklad_mn1;
			AOM_ask_value_double(Rev, "tpv4_SkladMnozstvi1", &sklad_mn1);
			sprintf(Nazvy[poradi][22], "%.2f", sklad_mn1);*/

			//07.03.2023
			/*double sklad_mn2;
			AOM_ask_value_double(Rev, "tpv4_SkladMnozstvi2", &sklad_mn2);
			sprintf(Nazvy[poradi][23], "%.2f", sklad_mn2);

			int klic_tpv;
			AOM_ask_value_int(Rev, "tpv4_KlicTPV", &klic_tpv);
			sprintf(Nazvy[poradi][24], "%d", klic_tpv);
*/
...
int ListBomLine(tag_t BomLine, int Level, tag_t pamet[], int poradi,tag_t BomWindow,int Strom [6][4])
{
	...
	char* kp_assembly_report="";
		...
		Rev = ask_item_revision_from_bom_line(pamet[Level]);
		...
		get_string(pamet[Level], "bl_item_item_id", &I_ID_v);
		get_string(pamet[Level], "bl_rev_item_revision_id", &REV_ID_v);
	...
	for(int t=0;t<ChildsCount;t++)
	{
		...
		RevThisLine = ask_item_revision_from_bom_line(Childs[t]);
	...
	//	BOM_line_ask_attribute_string(Childs[t], AttributeId, &tmpQuant);
		get_string(Childs[t], "bl_quantity", &tmpQuant);
		...
		//AOM_ask_value_tag(Childs[j], "bl_revision", &RevNew);
			RevNew = ask_item_revision_from_bom_line(Childs[j]);
		...
		get_string(Childs[t], "bl_sequence_no", &findNo);
	...
	//	poradi=ReadAttr_Projekt(RevThisLine, poradi, I_ID_v, REV_ID_v, Quant, findNo);
		if (debug) printf ("line: %d \n",__LINE__);
		//strcpy(Nazvy[poradi][7],tmp);
			char tmpName[128];
...
void ReadObjInRelation(tag_t Rev, int poradi)
{
	ECHO(("L %d------------------read Realation------------------poradi %d \n",__LINE__,poradi));
	int AttributeId = 0;
	char* tmp;
	char id[32];
	//double Hmotnost;
	...
	int num_rel = CountInRelation(Rev, relation, &obj_inRel);
	ECHO(("L:%d num_rel: %d \n", __LINE__,num_rel));
	if (num_rel != 0)
	{
		ECHO(("line: %d \n", __LINE__));
		...
		{
			ECHO(("line: %d \n", __LINE__));
			ITEM_ask_item_of_rev(obj_inRel[i], &Item);
			get_string(Item, "item_id", &tmp);
			if (debug) ECHO(("line: %d \n", __LINE__));
			strcpy(id, tmp);
			if (debug) ECHO(("line: %d \n", __LINE__));
			//strcpy(Nazvy[poradi][1], id);//ItemID
			copy_string2varacha(id, poradi, 2);
...
//fprintf(fp,attr);
 ECHO(("L:%d radku %d sloupcu %d /n", __LINE__, m, n));
 ECHO(("L:%d mode %s /n", __LINE__, mode));
for(i=0;i<m;i++)
{
	printf ("fprint %d \n",i);
	if(i==0){
		if (strcmp(mode, "projekt") == 0)
			fprintf(fp, "ItemID#ItemRevisionID#Nazev#Poznamka#TYP#Autor#Status#Stav#DatZpracovani#Schvaleno#Zakaznik\n");
		else if (strcmp(mode, "Vazby_Projekt_Vyrobek") == 0)
			fprintf(fp, "ItemID_Projekt#ItemRevisionID_Projekt#ItemID_Vyrobek#ItemRevisionID_Vyrobek\n");
		else if (strcmp(mode, "Vazby_Projekt_Pripravek") == 0)
			fprintf(fp, "ItemID_Projekt#ItemRevisionID_Projekt#ItemID_Pripravek#ItemRevisionID_Pripravek\n");
		else
    ...
    char  
		...
		*Pname="ren ",
		...
		while (ArgumentCount-- > 0)
		{
			...
			if (strcmp("RelationConnectObj", Flag) == 0)
			{
				ECHO(("RelationConnectObj %s \n", Value));
				strcpy(relation, Value);
			}
...
for( int i = 0; i < TargetsCount; i ++ )
{
	...
	for(int l=0;l<6;l++){
		...
		if (strcmp(relation, "0") != 0)
		{
			ReadObjInRelation(Targets[i], poradi++);
			ECHO(("L:%d \n", __LINE__));
		...
		}else if (BomsCount==0)
		{
			if (debug) printf ("line: %d \n",__LINE__);
			if(strcmp(mode,"projekt")==0)
				poradi=ReadAttr_Projekt(Targets[i], i, " ", " ", "1", "10");
			else
		...
		//	if (debug)ECHO(("L: %d file_name %s \n", __LINE__, file_name));
		//	if (strcmp(mode, "projekt") == 0)
		//		create_marks_csv(file_name, poradi, 9);
		//	else
		...
		//} else
			...
			{
				ECHO(("L:%d Projekt %s \n", __LINE__, mode));
				create_marks_csv(file_name, poradi, 3);
			}
			else if (strcmp(mode, "Vazby_Projekt_Pripravek") == 0)
			{
				ECHO(("L:%d Projekt %s \n", __LINE__, mode));
