...
int  Add_material(char* id_polozky,int *poradi,char* id_rodice,char* obj_name,char* pozice)
{
	...
	//printf ("test305 %s \n",ID_v);
		...
		/*AOM_ask_value_string(Rev,"tpv4_poznamka",&tmp);
		...
		//strcpy(Nazvy[poradi][10],tmp);//skupina
		////printf("item_rev %s \n",Nazvy[poradi][3]);
...
for(i=0;i<m;i++)
{
	printf ("fprint %d \n",i);
	if(i==0)
		fprintf(fp,"POZICE#IDV#ID#NAZEV#REVIZE#MNOZSTVI#PRILOHY#CV_ZAKAZNIK#ZAKAZNIK#POLOTOVAR#POZNAMKA#ATRIBUT_1#ATRIBUT_2#ATRIBUT_3#OBJEM#PLOCHA#HMOTNSOT#HUSTOTA#MATERIAL#TYP\n");
